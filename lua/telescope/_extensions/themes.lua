...
local function reload_theme(name)
  ...
  vim.api.nvim_exec_autocmds("User", { pattern = "OtsuThemeReload" })
end
...
local function switcher()
  ...
  local picker = pickers.new {
    prompt_title = "󱥚 Set Otsu Theme",
    previewer = previewer,
    finder = finders.new_table {
      results = require("otsu.utils").list_themes(),
    },
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      ------------ save theme to otsurc on enter ----------------
      actions.select_default:replace(function()
        if action_state.get_selected_entry() then
          local old_theme = dofile(vim.fn.stdpath "config" .. "/lua/otsurc.lua").ui.theme
          local selected_theme = action_state.get_selected_entry()[1]

          require("otsu.utils").replace_word(old_theme, selected_theme)
