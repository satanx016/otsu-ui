...
-- local otsu_types_fp = vim.fs.normalize(vim.fn.fnamemodify(debug.getinfo(1, "S").source:sub(2), ":p:h"))
local otsu_types_fp = fnamemodify(normalize(debug.getinfo(1, "S").source:sub(2)), ":p:h:h") .. "/otsu_types"
...
local gen_highlights = function()
  ...
  write_file(
    otsu_types_fp .. "/all_hl_groups.lua",
    table.concat(contents, "\n")
  )
end
